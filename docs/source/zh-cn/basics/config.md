...
root | 应用根目录，非本地或单元测试环境 HOME，其他环境为 baseDir
...
package 为一个 npm 模块，必须添加依赖到 pkg.dependencies 中。框架会在 node_modules 中找到这个模块作为插件入口。
...
module.exports = {
  mysql: {
    enable: true,
    path: path.join(__dirname, '../node_modules/egg-mysql'),
  },
};
```

path 为一个绝对路径，这样应用可以把自己写的插件直接放到应用目录中。
